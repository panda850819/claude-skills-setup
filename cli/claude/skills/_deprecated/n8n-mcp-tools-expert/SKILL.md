...
> 參考 Anthropic Engineering: "Code Execution with MCP" (Nov 2024)

### 核心概念

**問題**：直接工具呼叫消耗大量 context
**解決**：讓 agent 編寫代碼來批量執行工具操作

### 適用場景

| 場景 | 傳統方式 | MCP Code Execution |
|------|----------|-------------------|
| 批量建立節點 | N 次 addNode 呼叫 | 1 次 Code 節點執行迴圈 |
| 條件式連線 | 手動判斷 + 多次呼叫 | Code 節點動態生成 |
| 複雜驗證 | 多次 validate 呼叫 | Code 節點批量檢查 |

### 實作模式

```javascript
// 在 n8n Code 節點中批量操作
const nodeTypes = ['slack', 'discord', 'telegram'];
const results = [];

for (const type of nodeTypes) {
  // 透過 MCP 呼叫 n8n API
  const config = await $mcp.call('get_node_essentials', {
...
```
[Webhook/Trigger] → [Code 節點: MCP 批量操作] → [結果處理]
```

這種模式特別適合：
1. Workflow 自動修復（批量驗證 + 修正）
2. 節點批量配置（從 template 展開）
3. 動態 workflow 生成（根據輸入決定結構）
