...
upstream:
  - skill: n8n-workflow-patterns
    receives: workflow 需要表達式
  - skill: n8n-node-config
    receives: node 設定需要表達式
...
- **Code Node 語法** - 用 n8n-code (Code 不用 `{{}}`)
- **Workflow 設計** - 用 n8n-workflow-patterns
- **Node 設定** - 用 n8n-node-config
...
- **n8n Code**: When expressions aren't enough, use Code nodes
...
**Need Help?** Reference the n8n expression documentation or test directly in the n8n Expression Editor.
...
| 情況 | 行動 |
|------|------|
| Expression 顯示為文字 | 確認有 `{{}}` |
| 無法存取 webhook 資料 | 檢查是否用 `.body` |
| Node 參照失敗 | 確認名稱大小寫完全一致 |
| 複雜邏輯需求 | 建議改用 Code Node |
