...
| 情況 | 行動 |
|------|------|
| Sharpe > 3.0 | **警告** - 可能過擬合，需額外驗證 |
| Max DD > 30% | **警告** - 風險過高，建議調整 |
| 交易數 < 30 | **警告** - 統計意義不足 |
| 數據不足 | 建議延長測試期間或換資產 |
...
**Ready for**:
- [ ] pine-optimizer (if performance issues)
- [ ] pine-publisher (if validated)
